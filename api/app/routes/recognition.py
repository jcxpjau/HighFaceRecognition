...
async def asyncRecognition(
    ...
    face_locations = await run_in_threadpool(face_recognition.face_locations, image)

    if not face_locations:
        raise HTTPException(status_code=400, detail="No faces found.")

    unknown_encoding = (await run_in_threadpool(face_recognition.face_encodings, image, face_locations))[0]
